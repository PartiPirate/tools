    ...
    public function logSso($userId) {
    	...
    	try {
    		...
    		$select->columns(
    				...
    				$select->where(array(self::UK => $userId));
    	
    				$results = $zdb->execute($select);
    				if ( $results->count() == 0 ) {
    					Analog::log(
    							'No entry found for userId `' . $userId . '`',
    							Analog::WARNING
    							);
    					return false;
    				} else {
    					...
    					Analog::log('User `' . $row->login_adh . '` logged in via SSO.', Analog::INFO);
